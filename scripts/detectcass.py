...
class image_proc():
    ...
    def imdecode(self):
        global flag
        self.logo_cascade = cv2.CascadeClassifier('/home/anmol/catkin_ws/src/vitarana_drone/scripts/data/cascade.xml')
        time.sleep(0.05)
        ...
        if len(logo) is not 0:
                ...
                for (x, y, w, h) in logo:
                      ...
                      if flag == 0:
                          flag = 1
                          self.x_center = x + ( w - x) / 2
                          self.y_center = y + (h - y) / 2
            
  
        self.detect_confirm_pub.publish(self.confirmation_msg)
        print(self.confirmation_msg)

    def error_finder(self):
        focal_length=(200)/math.tan(1.3962634/2)
        self.x_err=(self.x_center*self.z_m)/focal_length-self.x_quad
        self.y_err=(self.y_center*self.z_m)/focal_length-self.y_quad
