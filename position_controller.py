...
class Position():
    ...
    def __init__(self):
        ...
        self.currxypub=rospy.Publisher('/quadxy',String,queue_size=1)
        # ------------------------------------------------------------------------------------------------------------
    ...
    def confirm_cordinates(self, data):
        ...
        if strdata[0] == 'True' and strdata[1]!="inf" and strdata[1]!="-inf":
    ...
    def Search_pattern(self):
        ...
        self.Fibonacci()
        if counter == 0:
            self.waypoint[2] = self.pickuploc[2] + 2
        if counter % 4 == 0:
    ...
    def pickup(self):
        ...
        if self.t == 1:
            ...
            # Hover 0.7 m above until coordinates on box are detected
                ...
                self.detectconf = False
                self.detectedcoord = np.array([0,0,0])
...
ng
        ...
        if ((abs(self.error[0]) < 0.1 and abs(self.error[1]) < 0.1 and abs(self.error[2]) < 0.1)) and self.detectconf is False:
            #self.waypoint[2]=self.currentlocxy[2]+2
            self.Search_pattern()
        ...
        if self.detectconf is True and self.detectedcoord[0] < 10 and self.detectedcoord[0]>-10:
