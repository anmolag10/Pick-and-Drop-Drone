...
class Position():
	...
	# Ratio of waypoint distance to total distance (is equal to 1 when at final goal) 
        self.t = self.dt / d
	# Waypoint [x,y,z], z = 25.16 which is 3 m above the spawn location
        waypoint = np.array([((1-self.t)*x0 + self.t*x1), ((1-self.t)*y0 + self.t*y1), 25.16])
        return waypoint
	...
	# self.t == 1 implies that the current setpoint is the final goal
		...
		# Set the delivery_flag and reinitialise some parameters to generate new waypoints
	...
	# Main waypoint generating condition if no obstacle
            ...
            if ((abs(self.error[0]) < 1 and abs(self.error[1]) < 1 and abs(self.error[2]) < 0.1) or self.avoid_flag == 1) and self.t != 1:
		...
		# Generating waypoints to final goal with step size 18
                self.waypoint = self.waypoint_generator(self.pickuploc[0], self.pickuploc[1], self.detectedcoord[0], self.detectedcoord[1], 18)
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        # Call pickup function if delivery flag is 0 else call delivery function
