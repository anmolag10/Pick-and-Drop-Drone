...
class Edrone():
    ...
    def __init__(self):
        ...
        self.Kp = [22.0*0.06, 22.0*0.06 , 808.0*0.06 , 546.0*0.06]
        self.Ki = [9.0*0.008, 9.00*0.008 , 8.0*0.008 , 52.0*0.008]
        self.Kd = [22.0*0.03, 22.0*0.03 , 306.0*0.03, 2271.0*0.03]
        ...
        rospy.Subscriber('/edrone/gps', NavSatFix, self.drone_altitude_callback)      
    ...
    # ----------------------------Define callback function like roll_set_pid t 
    ...
    def pid(self):
        ...
        self.ItermPitch=((self.ItermPitch+self.error[0])*self.Ki[0]/self.sample_time)
        self.Pitch_Out= (self.Kp[0]*self.error[0] + self.ItermPitch + ((self.Kd[0]*self.sample_time)*(self.error[0] - self.prev_values[0])))
        self.pitchErrorMsg.data=self.error[0]
        self.prev_values[0]=self.error[0]

        self.ItermRoll=((self.ItermRoll+self.error[1])*self.Ki[1]/self.sample_time)
        self.Roll_Out= (self.Kp[1]*self.error[1] + self.ItermRoll+ ((self.Kd[1]*self.sample_time)*(self.error[1] - self.prev_values[1])))
        self.rollErrorMsg.data=self.error[1]
        self.prev_values[1]=self.error[1]

        self.ItermYaw=((self.ItermYaw+self.error[2])*self.Ki[2]/self.sample_time)
        self.Yaw_Out= (self.Kp[2]*self.error[2] + self.ItermYaw + ((self.Kd[2]*self.sample_time)*(self.error[2] - self.prev_values[2])))
        self.yawErrorMsg.data=self.error[2]
        self.prev_values[2]=self.error[2]

       
        self.ItermHeight=((self.ItermHeight+self.error[3])*(self.Ki[3]/self.sample_time))
        
        self.Altitude_Out=(self.Kp[3]*self.error[3] + self.ItermHeight)+((self.Kd[3]*self.sample_time)*(self.error[3] - self.prev_values[3]))
        self.altitudeErrorMsg.data=self.error[3]
        self.prev_values[3]=self.error[3]
        


        print('AltitudeOut:',self.Altitude_Out)
        print(self.boundCheck((self.converted_throttle+self.Altitude_Out-self.Roll_Out+self.Pitch_Out-self.Yaw_Out)))
        print(self.boundCheck((self.converted_throttle+self.Altitude_Out-self.Roll_Out-self.Pitch_Out+self.Yaw_Out)))
        print(self.boundCheck((self.converted_throttle+self.Altitude_Out+self.Roll_Out-self.Pitch_Out-self.Yaw_Out)))
        print(self.boundCheck((self.converted_throttle+self.Altitude_Out+self.Roll_Out+self.Pitch_Out+self.Yaw_Out)))
        

        self.pwm_cmd.prop1=self.boundCheck((self.converted_throttle+self.Altitude_Out-self.Roll_Out+self.Pitch_Out-self.Yaw_Out))
        self.pwm_cmd.prop2=self.boundCheck((self.converted_throttle+self.Altitude_Out-self.Roll_Out-self.Pitch_Out+self.Yaw_Out))
        self.pwm_cmd.prop3=self.boundCheck((self.converted_throttle+self.Altitude_Out+self.Roll_Out-self.Pitch_Out-self.Yaw_Out))
        self.pwm_cmd.prop4=self.boundCheck((self.converted_throttle+self.Altitude_Out+self.Roll_Out+self.Pitch_Out+self.Yaw_Out))
