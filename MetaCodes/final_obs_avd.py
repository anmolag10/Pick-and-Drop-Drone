...
class Position():
    ...
    def __init__(self):
        ...
        self.Kd2 = np.array([1665 * 0.3, 2265 * 0.3, 465 * 0.3])       
        ...
        self.next_setpoint=np.array([10000,10000,1000])
    ...
    def setpointcontrol(self, lat, lon):
        ...
        print("Theta:",theta)
        if theta>math.pi/2:
            x_next, y_next = x0 - 18 , y0 - m * 18
            return x_next, y_next, 25.16
        x_next, y_next = x0 + 18 , y0 + m * 18
        return x_next, y_next, 25.16
    ...
    def delivery(self):
            ...
            if (self.ranges > 3).all():
                ...
                if abs(self.del_error[0]) < 1 and abs(self.del_error[1]) < 1 and abs(self.del_error[2]) < 1:
                    flag = 0
                    if math.sqrt((self.next_setpoint[0]-self.lat_to_x(self.detectedcord[0]))**2 + (self.next_setpoint[1]-self.long_to_y(self.detectedcord[1]))**2) < 18:
                        
                        x = self.lat_to_x(self.detectedcord[0])
                        y = self.long_to_y(self.detectedcord[1])
                        z=25.16
                        flag = 1
                    if flag == 0:
                        x,y,z = self.setpointcontrol(self.detectedcord[0], self.detectedcord[1])
                    self.next_setpoint = np.array([x,y,z])
                self.del_error = np.round((self.next_setpoint - self.currentlocxy), 7)

            elif self.ranges[3] < 4 and self.ranges[3]>0.28:
                print("2")
                self.del_error = np.round(np.array([(1-self.ranges[3]), (self.currentlocxy[1]-1.5), (25.16-self.currentlocxy[2])]), 7)

            elif self.ranges[0] < 4 and self.ranges[0]>0.28:
                print("3")
                self.del_error = np.round(np.array([(self.currentlocxy[0]-1.5), (1-self.ranges[0]), (25.16-self.currentlocxy[2])]), 7)
