...
class Position():
    ...
    def waypoint_generator(self, lat1, lon1, lat2, lon2, dist):
        ...
        t = self.dt / d
        waypoint = np.array([((1-t)*x0 + t*x1), ((1-t)*y0 + t*y1), 27.16])
        return waypoint, t
    ...
    def delivery(self):
        ...
        if self.ranges[3] < 4.5:
            ...
            if ((abs(self.del_error[0]) < 1 and abs(self.del_error[1]) < 1 and abs(self.del_error[2]) < 0.1) or self.flag == 1) and self.t != 1:
                # if self.flag == 1:
                #     self.pickuploc = self.currentlocxy
                #     self.dt = 0
                #     self.t = 0
                #     self.flag = 0
                self.waypoint, self.t = self.waypoint_generator(self.pickuploc[0], self.pickuploc[1], self.detectedcord[0], self.detectedcord[1], 2)
   
        
        
        # elif self.ranges[1] < 4:
            
        #     if self.flag == 0:
        #         self.flag = 1
        #     # self.del_error = np.round(np.array([-1.5, (2-self.ranges[0]), (25.16-self.currentlocxy[2])]), 7)
        #     # self.waypoint, _ = self.waypoint_generator(self.pickuploc[0], self.pickuploc[1], self.detectedcord[0], self.detectedcord[1], 2)
        #     self.waypoint[0] = self.currentlocxy[0] 
        #     self.waypoint[1] = self.currentlocxy[1] + 3
